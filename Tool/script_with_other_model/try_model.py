...
def get_embedding_model(model_choice):
    ...
    logging.info(f"Caricamento del modello di embedding: {model_map[model_choice]}")
    return SentenceTransformer(model_map[model_choice])

# Carica un modello leggero per generazione testo (chat completion)
...
def preprocess_prompts(prompts, embedding_model):
...
def classify_prompt(new_prompt, prompts, embeddings, regressor, cwe_mapping, embedding_model):
    logging.info(f"Classifying new prompt: {new_prompt}")
    new_embedding = get_embedding(new_prompt, embedding_model)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Classify prompts and find CWE vulnerabilities using different Sentence-Transformers models.")
    ...
    parser.add_argument("--model", type=str, default="minilm", 
                        help="Scegli il modello di embedding: 'minilm', 'mpnet', 'distilroberta' (default: minilm)")
    args = parser.parse_args()

    # Carica il modello di embedding in base alla scelta
    ...
    for prompt in new_prompts:
        score, similarity, cwe = classify_prompt(prompt, prompts, embeddings, regressor, cwe_mapping, embedding_model)
        print(f"\nPrompt: {prompt}")
